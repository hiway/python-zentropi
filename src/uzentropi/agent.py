...
class Frame(object):
    ...
    @staticmethod
    def inflate(frame):
        _frame = {'kind': Frame.UNSET, 'id': None, 'name': None, 'data': {}, 'meta': {}}
        _frame.update(frame)
        return _frame
    ...
    def load(frame: dict):
        ...
        if kind == Frame.EVENT:
            print('INFLATED', Frame.inflate(frame))
            return Event(**Frame.inflate(frame))
        else:
            print('*** skipping frame', frame)
    ...
    @staticmethod
    def event(*, name, data=None, meta=None, id=None):
        return Event(id=id, name=name, data=data, meta=meta, kind=Frame.EVENT)


Event = namedtuple('Event', ['id', 'name', 'data', 'meta', 'kind'])
    ...
    def _process(self, frame):
        ...
        if isinstance(frame, Event):
            try:
                ...
                # raise KeyError('Malformed event; no `name` specified: {}'.format(frame))
                print('*** skipping:', frame)
        else:
            print('*** skipping:', frame)

    async def connect(self, endpoint, frame_handler, auth):
        ...
        print('*** auth', auth)
        if auth:
            endpoint = endpoint.format(auth=auth)
        print('*** connecting to', endpoint)
        with websockets.client.connect(endpoint) as websocket:
...
class Agent(object):
    ...
    def emit(self, name, data=None):
        event = Frame.event(name=name, data=data or {}, id=str(time.ticks_us()), meta={})
    ...
    def connect(self, endpoint, connection_class=None):
        ...
        self.spawn(self._connection.connect(endpoint, self._trigger_event_handler, auth=self._auth))
